...
class FrameStackPolicyNetwork(nn.Module):
    ...
    def __init__(self, n_feats=N_FEATURES, hidden_size=64, history_length=4, device="cpu"):
        ...
        self.network = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_size, n_feats * N_ACTIONS),
        )
        ...
        if is_batched:
            ...
            if available_history > 0:
                ...
                if history_frames.size(0) < expected_history_size:
                    ...
                    features = torch.cat([norm_prices, history_frames, norm_positions, norm_cash], dim=0)
            else:
                features = torch.cat([
                    norm_prices,
            ...
            logits = self.network(features.unsqueeze(0)).squeeze(0).reshape(self.n_stocks, N_ACTIONS)
