...
class Nof1RolloutBuffer(RolloutBuffer):
    """
    ...
    def reset(self) -> None:
        ...
        for key, obs_input_shape in self.obs_shape.items():
            if key == 'map':
                self.observations[key] = np.zeros((self.buffer_size, self.n_envs, *(22,22,8)), dtype=np.float32)
            else:
    ...
    def get(  # type: ignore[override]
        ...
        assert self.full, ""
        indices = sorted(np.random.permutation(self.buffer_size * self.n_envs))
        # Prepare the data
...
class Nof1PPO(OnPolicyAlgorithm):
    """
