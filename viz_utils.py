...
def grid_archive_heatmap(archive,
    ...
    validate_heatmap_visual_args(
        aspect, cbar, archive.measure_dim, [1, 2, 3],
        "Heatmap can only be plotted for a 1D, 2D or 3D GridArchive")
    ...
    if archive.measure_dim == 1:
    ...
    elif archive.measure_dim == 3:
        x_dim, y_dim, z_dim = archive.dims
        x_bounds, y_bounds, z_bounds = archive.boundaries

        # Prepare a full 3D grid of NaNs
        colors = np.full((z_dim, y_dim, x_dim), np.nan)
        grid_index_batch = archive.int_to_grid_index(index_batch)

        for i in range(len(index_batch)):
            x, y, z = grid_index_batch[i]
            colors[z, y, x] = objective_batch[i]

        if transpose_measures:
            # Flip x/y axes in each 2D slice if requested
            colors = colors.transpose((0, 2, 1))  # z, x, y
            x_bounds, y_bounds = y_bounds, x_bounds
        ...
        for z in range(z_dim):
            ...
            t = ax.pcolormesh(
                ...
                slice_colors,
                cmap=cmap,
                vmin=np.nanmin(objective_batch) if vmin is None else vmin,
                vmax=np.nanmax(objective_batch) if vmax is None else vmax,
                rasterized=rasterized,
                **(pcm_kwargs or {})
            )
            if cbar == "auto":
